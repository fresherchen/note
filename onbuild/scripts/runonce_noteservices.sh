...
NOTES_SESSION_SECRET=${NOTES_SESSION_SECRET:-$NOTES_DBNAME}
...
runonce_noteservices() {
    ...
    if [[ -n "DB_PORT_27017_TCP_ADDR}" ]]; then
        sudo -u node -H sed -i "s/{{NOTES_DBHOST}}/${DB_PORT_27017_TCP_ADDR}/" ${APP_DIR}/config/env/${NODE_ENV}.js
        # local.example.js
        sudo -u node -H sed -i "s/{{NOTES_DBHOST}}/${DB_PORT_27017_TCP_ADDR}/" ${APP_DIR}/config/env/local.example.js
    fi
